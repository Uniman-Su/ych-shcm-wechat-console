function refreshToken(){
	...
	var exp = sessionStorage.getItem('putexp');
	var now = new Date().getTime();
	var time = parseInt(((exp - now) / 1000 - 600) * 1000);
	//console.log(time);
	setTimeout(function() {
		token = sessionStorage.getItem('puttoken');
		mui.ajax({
			type: "post",
			...
			success: function(data) {
				...
				sessionStorage.setItem('putexp', newexp);
				sessionStorage.setItem('puttoken', newToken);
				/*token = newToken;
				exp = newexp;*/
			}
		});
